...
void get_p2() {
    ...
    if(file.is_open()) {
        ...
        stack<char> temp;
        for(int i = 0; i < amount; i++) {
            temp.push(cargo[from].back());
            cargo[from].pop_back();
        }
        while (!temp.empty()) { 
            cargo[to].push_back(temp.top());
            temp.pop();
        }
        }
    }
...
void initialise_cargo() {
    vector<deque<char>> temp_cargo(10);
    for (int i = 1; i <= 9; i++) {
        deque<char> temp;
        temp_cargo.push_back(temp);
    }
    cargo = temp_cargo;
    ...
    if(file.is_open()) {
        while(getline(file, line)) {
            ...
            for (int i = 0; i < line.size(); i++) {
                if(line[i] == '[') {
                    cargo[i / 4 + 1].push_front(line[i + 1]);
                }
